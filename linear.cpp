...
class y_avltree_rank_fun: public y_rbtree_rank_fun
{
	public:
		...
		double fun(double *w);
		double _fun(double *w,model*model_);
...
double y_avltree_rank_fun::_fun(double *w,model*model_)
{
	int i,j,k;
	double f=0;
	double V_l2 = 0;
	...
	for(i=0;i<w_size;i++)
		f += w[i]*w[i];
	f /= 2.0;
	////////////////////////////////we consider V as a regular term
	for(i=0;i<l;i++)
		for(j=0;j<col_size;j++)
			V_l2 += v_pp[i][j]*v_pp[i][j];
	//f += V_l2;
	//////////////////////////////////////////////////////////////////
	//calculate pw	
	for(i=0;i<l;i++)
	...
	for(i=0;i<l;i++)
		loss_term += (l_minus[i] - l_plus[i])*z[i];
	f += C*(pw-loss_term);	
	return(f);
}
void y_avltree_rank_fun::_grad(model *model_,const problem *prob,int nr_subset, int *perm, int *start, int *count)
{
    ...
    for(it=s.begin();it!=s.end();it++) 
	{
		...
		////////////////////////////consider matrix V's l2 normalization as a regularization///////////////////
		//for(int i=0;i<col_size;i++)
			//v_grad_pp[batch_count][i] += v_pp[v_index][i];
		////////////////////////////consider matrix V's l2 normalization as a regularization///////////////////
